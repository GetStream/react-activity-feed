...
export class StreamApp extends React.Component<StreamAppProps, StreamAppState> {
  ...
  constructor(props: StreamAppProps) {
    ...
    this.state = StreamApp.initClientState(props, {
      changedUserData: () => {
        this.setState({ userData: this.state.userData }); // was user.data
      },
      t: null,
  ...
  static initClientState = function<S>(props: StreamAppProps, state: S) {
    ...
    return newState as typeof newState & S;
  };
  ...
  getUserInfo = async () => {
    try {
      ...
      this.props.errorHandler(e, 'get-user-info', {
        userId: this.state.client.userId, // TODO type issue: this.state.user.id - doesn't exist?
      });
      return;
    }
