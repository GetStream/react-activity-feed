...
type FeedManagerState = {
  ...
  reactionActivities: { [key: string]: string };
  // Used for finding reposted activities
  activityIdToPaths: { [key: string]: Array<string[]> };
  reactionIdToPaths: { [key: string]: Array<string[]> };
  unread: number;
  unseen: number;
...
export class FeedManager {
  ...
  setState = (
    changed:
      | Partial<FeedManagerState>
      | ((FeedManagerState) => Partial<FeedManagerState>),
  ) => {
  ...
  onAddReaction = async (
    ...
    const enrichedReaction = immutable.fromJS({
      ...reaction,
      user: this.props.user, // was user.full
    });
  ...
  _onMarkAs = async (
    ...
    let markArg: any = group;
    if (group === true) {
      groupArray = this.state.activityOrder;
    } else if (Array.isArray(group)) {
      groupArray = (group as Array<any>).map((g) => g.id);
      markArg = groupArray;
    } else {
      markArg = group.id;
      groupArray = [group.id];
  ...
  doFeedRequest = async (options: FeedRequestOptions): Promise<FR> => {
    ...
    return (await this.feed().get(options)) as FR;
  };
    ...
    if (this.props.notify) {
      ...
      await this.setState((prevState) => {
        ...
        subscription.then(
          () => {
            console.log(
              `now listening to changes in realtime for ${this.props.feedGroup}`,
            );
          },
          (err) => {
            console.error(err);
  ...
  unsubscribe = async () => {
    ...
    if (this.registeredCallbacks.length === 0) {
      try {
        ...
        console.log(
          `stopped listening to changes in realtime for ${this.props.feedGroup}`,
        );
      } catch (err) {
        console.error(err);
      }
  ...
  loadNextPage = async () => {
    ...
    const nextURL = new URL(lastResponse.next, true) as URL;
