...
export default class IconBadge extends React.Component {
  render() {
    return (
      <div className="raf-icon-badge" role="button" onClick={this.props.onClick}>
        {this.props.children ? (
          this.props.children
        ) : (
        ...
        {!this.props.hidden && this.props.unseen > 0 ? (
          <div className="raf-icon-badge__badge">{this.props.showNumber ? <p>{this.props.unseen}</p> : null}</div>
        ) : null}
      </div>
    );
  }
